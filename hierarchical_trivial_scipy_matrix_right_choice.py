...
# chosen_type corresponds to the segmentation type we want to use for clustering:
# 5- no segmentation correction is performed, we use segmentation performed on 2 images
# 6- segmentation used in our article
# 8- GT segmentation
for chosen_type_seg in [5, 6, 8]:
    already_computed = False    # if the clustering is already performed and we only want to recalculated stats
    if S2:
        apply_mask_outliers = False
    else:
        apply_mask_outliers = True

    # Choose the right one
    metric_type = "mah" # choice between "eucl"(euclidean) and "mah"(mahalanobis) distances for clustering
    metric_type = "eucl" # choice between "eucl"(euclidean) and "mah"(mahalanobis)




    if chosen_type_seg == 5:
