...
def main():
    ...
    parser.add_argument('--nb_features', default=10, type=int)
    parser.add_argument('--batch_size', default=150, type=int)
    parser.add_argument('--bands_to_keep', default=4, type=int)
    parser.add_argument('--epoch_nb', default=1, type=int)
    parser.add_argument('--learning_rate', default=0.0001, type=float)
    parser.add_argument('--noise_factor', default=0.25, type=float, help='for denoising AE')
    parser.add_argument('--noise_factor_ndvi', default=None, type=float, help='for denoising AE')
    parser.add_argument('--centered', default=True, type=bool)
    parser.add_argument('--mask', default=False, type=bool)
    parser.add_argument('--original_layers', default=[32, 32, 64, 64], type=list, help='Nb of conv. layers to build AE')
    parser.add_argument('--ndvi_layers', default=[16,16, True], type=list, help='Nb of conv. layers to build AE and pooling option')
    args = parser.parse_args()
    # args.nb_features = args.nb_clusters
    # print(args)
    #
    ...
    path_results_nn = os.path.expanduser(
        '~/Desktop/Results/RESULTS_CHANGE_DETECTION/NN_Montpellier_SPOT5_all_images_model_pretrained/All_images_ep_5_patch_5_fc.2019-02-11_1555/threshold_int_0.4/Filled_holes/')
    ...
    print_stats(stats_file, "No l2 norm!!!", print_to_console=True)
    ...
    if args.mask is True:
        ...
        list_outliers_array = np.zeros((nbr_images-1, H, W), dtype=int)
        outliers_folders = np.sort(np.asarray([f for f in os.listdir(path_results_nn) if f.startswith("Joint_AE_")]))
        # print(outliers_folders)
        for folder in range(len(outliers_folders)):
            f = outliers_folders[folder]
            outliers_file = [fi for fi in os.listdir(path_results_nn+f) if fi.startswith("Outliers_") and fi.endswith(".TIF")][0]
            outliers_array, H, W, _, _, _ = open_tiff(path_results_nn+f, os.path.splitext(outliers_file)[0])
            list_outliers_array[folder] = outliers_array
        outliers_total = np.sum(list_outliers_array, axis=0)
        outliers_total_new = np.zeros_like(outliers_total)
        # outliers_total_new[outliers_total == 0] = 1
        # create_tiff(1, path_results_final + "Outliers_total", W, H, gdal.GDT_Byte, np.reshape(outliers_total_new, (H, W)), geo, proj)
        mask = np.where(outliers_total.flatten() == 0)[0]




    if args.mask is False:
        mask = np.arange(H*W)
    ...
    nbr_patches_per_image = int(len(mask))
    print_stats(stats_file, "Nbr of training patches  "+str(nbr_patches_per_image), print_to_console=True)
    image = ImageDataset(list_image_extended_tr, list_image_extended_ndvi_tr, args.patch_size, args.patch_size_ndvi, sample(list(mask), nbr_patches_per_image)) #we create a dataset with tensor patches
    ...
    mask = np.arange(H*W)
    image = ImageDataset(list_image_extended_tr, list_image_extended_ndvi_tr, args.patch_size, args.patch_size_ndvi, list(mask))  # we create a dataset with tensor patches
    try:
        batch_size = W
    ...
    new_encoded_array = np.zeros((H * W, encoded_array.shape[-1]))-9999
    new_encoded_array[mask]= encoded_array
    print(new_encoded_array.shape)
    new_encoded_array = np.transpose(new_encoded_array, (1, 0))
    ...
    total_time_pretraining = end_time - start_time
    total_time_pretraining = str(datetime.timedelta(seconds=total_time_pretraining))
    print_stats(stats_file, "Total time pretraining =" + str(total_time_pretraining) + "\n")
    # epoch_stats = ["best epoch " + str(best_epoch), "best epoch loss " + str(best_epoch_loss)]
    # print_stats(stats_file, epoch_stats)
